...
export async function GET() {
  ...
  try {
    ...
    let importance = numericCols.map((name, i) => ({
      name,
      importance: Math.abs(pearson(matrix[i], targetVec)),
    }));
    importance.sort((a, b) => b.importance - a.importance);
    // 타겟 변수(quality_defect 등)는 자기 자신과 상관 1이라 목록에서 제외
    ...
    if (defectCol && map.dateCol) {
      ...
      const [trendRows]: any = await conn.query(
        `SELECT 
          DATE_FORMAT(${escapeSqlId(map.dateCol)}, '%Y-%m-%d %H:00:00') as time_hour,
          AVG(${escapeSqlId(defectCol)}) as avg_rate
         FROM ${escapeSqlId(processTable)}
         WHERE ${escapeSqlId(defectCol)} IS NOT NULL
           AND ${escapeSqlId(map.dateCol)} >= DATE_SUB(NOW(), INTERVAL 7 DAY)
         GROUP BY DATE_FORMAT(${escapeSqlId(map.dateCol)}, '%Y-%m-%d %H:00:00')
