...
## 0. 로그인 동작을 위한 환경 변수 (필수)

**로그인/회원가입**은 Vercel의 API 라우트(`/api/auth/*`)에서 처리되므로, 아래 변수를 반드시 설정하세요.

1. [Vercel](https://vercel.com) → 프로젝트 → **Settings** → **Environment Variables**
2. 추가할 변수:
   - **`JWT_SECRET`** (필수): 로그인 세션 토큰 서명용. 32자 이상 랜덤 문자열 권장.
   - **`DB_HOST`**: MariaDB/MySQL 호스트 (예: Lightsail DB 또는 RDS 주소). 미설정 시 `localhost`로 시도되어 연결 실패할 수 있음.
   - **`DB_USER`**, **`DB_PASSWORD`**, **`DB_NAME`**: DB 접속 정보. (또는 `AUTH_DB_*` 사용 가능)
3. **Save** 후 **Redeploy** 실행.

> ⚠️ DB가 Vercel에서 접근 가능한 공인 IP/호스트여야 합니다. Lightsail DB 사용 시 방화벽에서 Vercel IP 또는 0.0.0.0/0 허용이 필요할 수 있습니다.
